...
const app: AppEntry = { folder: "_instances/webmailx", name: "Roundcube", enabled: true };
...
describe("applyRoundcubeUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(path.join(appsDir, "_instances", "webmailx", "config"), { recursive: true });
    await writeFile(
      path.join(appsDir, "_instances", "webmailx", "config", "config.inc.php"),
      "CUSTOM_CONFIG",
    );
    await writeFile(path.join(appsDir, "_instances", "webmailx", "OLD_FILE.txt"), "keep me");
  ...
  it("overlays new files without touching an existing config/config.inc.php or unrelated files", async () => {
    ...
    await expect(
      readFile(path.join(appsDir, "_instances", "webmailx", "config", "config.inc.php"), "utf8"),
    ).resolves.toBe("CUSTOM_CONFIG");
    await expect(readFile(path.join(appsDir, "_instances", "webmailx", "NEW_FILE.txt"), "utf8")).resolves.toBe(
      "brand new",
    );
    expect(existsSync(path.join(appsDir, "_instances", "webmailx", "OLD_FILE.txt"))).toBe(true);
    expect(
      existsSync(path.join(appsDir, "_instances", "webmailx", "config", "config.inc.php.sample")),
    ).toBe(true);
  });
  ...
  it("throws on checksum mismatch and leaves the target untouched", async () => {
    ...
    expect(existsSync(path.join(appsDir, "_instances", "webmailx", "NEW_FILE.txt"))).toBe(false);
  });
});
